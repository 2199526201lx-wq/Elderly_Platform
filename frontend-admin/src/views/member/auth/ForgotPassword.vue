    ...
    <div class="login-card">
      ...
      <el-form ref="formRef" :model="form" :rules="rules" style="margin-top: 24px">
        <el-form-item prop="phone">
          <el-input v-model="form.phone" placeholder="注册手机号" :prefix-icon="Iphone" size="large" />
        </el-form-item>
        ...
        <el-form-item prop="newPassword">
          <el-input v-model="form.newPassword" placeholder="新密码" type="password" :prefix-icon="Lock" size="large" show-password />
        </el-form-item>
        <el-form-item>
...
async function sendCode() {
  if (!form.value.phone) { ElMessage.warning('请先输入手机号'); return }
  try {
...
async function handleReset() {
  ...
  loading.value = true
  try {
    await request.post('/api/auth/reset-password', { phone: form.value.phone, code: form.value.code, newPassword: form.value.newPassword })
    ElMessage.success('密码重置成功'); router.replace('/member/login')
